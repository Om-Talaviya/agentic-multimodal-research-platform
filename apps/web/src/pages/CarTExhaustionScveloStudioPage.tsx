...
export const CarTExhaustionScveloStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Single-Cell RNA Velocity Lineage Trajectory & CAR-T Epigenetic Exhaustion Interceptor Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/car-t-exhaustion-scvelo/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Single-Cell RNA Velocity Lineage Trajectory & CAR-T Epigenetic Exhaustion Interceptor Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 220: Autonomous Simulates spliced-to-unspliced mRNA turnover kinetics to construct dynamic RNA velocity vectors, identifying critical branching points where CAR-T cells diverge into terminal exhaustion.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">exhaustion_diversion_efficiency_pct</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.exhaustion_diversion_efficiency_pct}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">stem_memory_persistence_index</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.stem_memory_persistence_index}</p>
                </div>
              </div>
