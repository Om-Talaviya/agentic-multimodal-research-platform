...
export const CryoEMFocusedRefinementStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Cryo-EM Focused Refinement & Deep Symmetrization Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/cryoem-focused-refinement/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Cryo-EM Focused Refinement & Deep Symmetrization Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 197: Autonomous Cryo-EM Sub-Volume Masked Focused Refinement, Local B-Factor Sharpening & Non-Crystallographic Symmetry (NCS) Expansion
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">local_resolution_angstroms</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.local_resolution_angstroms}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">map_to_model_cross_correlation</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.map_to_model_cross_correlation}</p>
                </div>
              </div>
