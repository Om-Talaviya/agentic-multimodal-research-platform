...
export const CRISPRCas13RNATargetingStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('CRISPR-Cas13 RNA-Targeting & Collateral Cleavage Suppressor Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/crispr-cas13-rna-targeting/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              CRISPR-Cas13 RNA-Targeting & Collateral Cleavage Suppressor Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 195: Autonomous CRISPR-Cas13 High-Affinity RNA Cleavage, HEPN Catalytic Domain Activation & Non-Specific Collateral Degradation Suppression
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">on_target_rna_knockdown_percent</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.on_target_rna_knockdown_percent}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">collateral_rna_suppression_ratio</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.collateral_rna_suppression_ratio}</p>
                </div>
              </div>
