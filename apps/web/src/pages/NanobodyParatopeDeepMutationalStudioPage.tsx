...
export const NanobodyParatopeDeepMutationalStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Heavy-Chain Nanobody (VHH) Paratope Deep Mutational Scanning (DMS) & Conformational Thermal Stability Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/nanobody-paratope-deep-mutational/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Heavy-Chain Nanobody (VHH) Paratope Deep Mutational Scanning (DMS) & Conformational Thermal Stability Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 223: Autonomous Predicts CDR3 paratope saturation mutagenesis fitness landscapes, calculating binding affinity DeltaDeltaG and melting temperature shifts for heavy-chain single-domain antibodies.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">thermal_melting_shift_celsius</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.thermal_melting_shift_celsius}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">affinity_kd_enrichment_fold</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.affinity_kd_enrichment_fold}</p>
                </div>
              </div>
