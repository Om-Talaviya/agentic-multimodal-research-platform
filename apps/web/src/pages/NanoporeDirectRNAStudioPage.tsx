...
export const NanoporeDirectRNAStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Nanopore Direct RNA Sequencing & Epitranscriptomic Modification Mapper Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/nanopore-direct-rna/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Nanopore Direct RNA Sequencing & Epitranscriptomic Modification Mapper Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 189: Autonomous Nanopore Direct RNA Sequencing, Ionic Current Squiggle Dwell-Time & Epitranscriptomic m6A/m5C Modification Mapping
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">median_polya_tail_length_nt</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.median_polya_tail_length_nt}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">epitranscriptomic_modification_stoichiometry</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.epitranscriptomic_modification_stoichiometry}</p>
                </div>
              </div>
