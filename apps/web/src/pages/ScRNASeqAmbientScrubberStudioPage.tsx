...
export const ScRNASeqAmbientScrubberStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Single-Cell RNA-seq Droplet De-multiplexing & Ambient RNA Scrubber Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/scrnaseq-ambient-scrubber/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Single-Cell RNA-seq Droplet De-multiplexing & Ambient RNA Scrubber Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 202: Autonomous Droplet-Based Single-Cell RNA-seq Cell-Free Ambient RNA Soup Decontamination, Doublet Deconvolution & Sample Multiplexing
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">ambient_rna_contamination_fraction</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.ambient_rna_contamination_fraction}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">doublet_detection_auc</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.doublet_detection_auc}</p>
                </div>
              </div>
