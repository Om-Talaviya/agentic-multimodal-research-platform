...
export const SyntheticGeneToggleSwitchStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Synthetic Gene Circuit Toggle Switch & Stochastic Noise Forecaster Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/synthetic-gene-toggle-switch/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Synthetic Gene Circuit Toggle Switch & Stochastic Noise Forecaster Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 205: Autonomous Synthetic Biological Bistable Toggle Switch Circuit, Chemical Master Equation Gillespie Stochastic Simulation & Bimodality Noise Forecasting
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">bistable_switching_threshold_molecules</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.bistable_switching_threshold_molecules}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">stochastic_fano_factor_noise</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.stochastic_fano_factor_noise}</p>
                </div>
              </div>
