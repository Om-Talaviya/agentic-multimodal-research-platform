...
export const SpatialMassCytometryImcStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Spatial Imaging Mass Cytometry (IMC) 40-Plex Phenotyping & Microenvironment Niche Ranker Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/spatial-mass-cytometry-imc/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Spatial Imaging Mass Cytometry (IMC) 40-Plex Phenotyping & Microenvironment Niche Ranker Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 221: Autonomous Processes metal-isotope tagged antibody imaging mass cytometry laser ablation channels, performing multi-channel cell segmentation, deep phenotyping, and cell neighborhood scoring.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">segmentation_f1_score</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.segmentation_f1_score}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">phenotypic_purity_pct</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.phenotypic_purity_pct}</p>
                </div>
              </div>
