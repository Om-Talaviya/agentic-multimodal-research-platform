...
export const UbiquitinationE3SelectivityStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Proteome-Wide Ubiquitination & E3 Ligase Selectivity Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/ubiquitination-e3-selectivity/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Proteome-Wide Ubiquitination & E3 Ligase Selectivity Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 207: Autonomous Deep Learning Ubiquitin Lysine Conjugation Site Prediction, E3 Ligase Substrate Selectivity & PROTAC Degradomics
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">ubiquitination_site_prediction_auroc</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.ubiquitination_site_prediction_auroc}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">e3_ligase_selectivity_binding_affinity_kd_nm</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.e3_ligase_selectivity_binding_affinity_kd_nm}</p>
                </div>
              </div>
