...
export const MRNALNPEncapsulationStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Therapeutic mRNA LNP Encapsulation & Structure Thermodynamics Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/mrna-lnp-encapsulation/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Therapeutic mRNA LNP Encapsulation & Structure Thermodynamics Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 201: Autonomous Therapeutic mRNA Lipid Nanoparticle (LNP) Formulation Optimization, Microfluidic Mixing Flow Ratio & Secondary Structure Minimum Free Energy (MFE) Thermodynamics
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">encapsulation_efficiency_percent</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.encapsulation_efficiency_percent}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">polydispersity_index_pdi</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.polydispersity_index_pdi}</p>
                </div>
              </div>
