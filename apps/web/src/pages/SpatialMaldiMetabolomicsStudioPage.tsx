...
export const SpatialMaldiMetabolomicsStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Spatial Metabolomics MALDI-MSI Tissue Architecture Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/spatial-maldi-metabolomics/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Spatial Metabolomics MALDI-MSI Tissue Architecture Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 188: Autonomous Matrix-Assisted Laser Desorption/Ionization (MALDI-MSI) Oncometabolite Spatial Tissue Architecture
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">spatial_resolution_microns</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.spatial_resolution_microns}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">metabolic_heterogeneity_index</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.metabolic_heterogeneity_index}</p>
                </div>
              </div>
