...
export const CrisprPrimePegRnaFlapStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous CRISPR Prime Editing pegRNA Primer Binding Site (PBS) & Reverse Transcription Flap Kinetics Synthesizer Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/crispr-prime-peg-rna-flap/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous CRISPR Prime Editing pegRNA Primer Binding Site (PBS) & Reverse Transcription Flap Kinetics Synthesizer Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 222: Autonomous Synthesizes dual pegRNA/ngRNA configurations, optimizing PBS melting temperature and RT template length while suppressing competing 5'-flap non-homologous end joining.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">prime_editing_efficiency_pct</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.prime_editing_efficiency_pct}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">indel_byproduct_ratio_pct</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.indel_byproduct_ratio_pct}</p>
                </div>
              </div>
