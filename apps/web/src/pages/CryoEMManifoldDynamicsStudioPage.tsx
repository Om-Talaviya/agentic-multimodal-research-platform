...
export const CryoEMManifoldDynamicsStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Cryo-EM Continuous Conformational Heterogeneity & Manifold Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/cryoem-manifold-dynamics/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Cryo-EM Continuous Conformational Heterogeneity & Manifold Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 192: Autonomous Cryo-EM Single-Particle Continuous Conformational Heterogeneity, 3D Variability Analysis (3DVA) & Latent Manifold Embedding
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">latent_manifold_eigenvalue_variance</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.latent_manifold_eigenvalue_variance}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">free_energy_barrier_kcal_mol</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.free_energy_barrier_kcal_mol}</p>
                </div>
              </div>
