...
export const LongReadSVAssemblyStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Long-Read Structural Variant & De Novo Assembly Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/long-read-sv-assembly/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Long-Read Structural Variant & De Novo Assembly Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 208: Autonomous Whole-Genome Long-Read Sequencing De Novo Assembly, Complex Structural Variant Breakpoint Resolution & Haplotype Phasing
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">contig_n50_megabases</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.contig_n50_megabases}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">structural_variant_breakpoint_precision_bp</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.structural_variant_breakpoint_precision_bp}</p>
                </div>
              </div>
