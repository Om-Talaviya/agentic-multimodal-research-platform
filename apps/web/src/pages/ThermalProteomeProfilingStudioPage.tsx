...
export const ThermalProteomeProfilingStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Thermal Proteome Profiling & Target Engagement Deconvolution Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/thermal-proteome-profiling/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Thermal Proteome Profiling & Target Engagement Deconvolution Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 190: Autonomous Proteome-Wide Thermal Shift Assay (CETSA/TPP) Melting Curve Deconvolution & Intracellular Target Engagement
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">melting_temperature_shift_celsius</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.melting_temperature_shift_celsius}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">target_engagement_confidence_auc</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.target_engagement_confidence_auc}</p>
                </div>
              </div>
