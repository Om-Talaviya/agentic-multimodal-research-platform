...
export const TargetedCovalentInhibitorWarheadStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Targeted Covalent Inhibitor (TCI) Electrophilic Warhead Reactivity & Cysteine Residence Time Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/targeted-covalent-inhibitor-warhead/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Targeted Covalent Inhibitor (TCI) Electrophilic Warhead Reactivity & Cysteine Residence Time Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 216: Autonomous Evaluates electrophilic warhead acrylamide/haloacetamide reactivity with non-catalytic target cysteines, predicting kinome kinact/KI selectivity and target residence time.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">kinact_over_ki_M_s</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.kinact_over_ki_M_s}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">cysteine_residence_time_hours</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.cysteine_residence_time_hours}</p>
                </div>
              </div>
