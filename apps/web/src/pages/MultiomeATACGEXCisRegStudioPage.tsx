...
export const MultiomeATACGEXCisRegStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Single-Cell Multiome ATAC+GEX Peak-to-Gene Cis-Regulatory Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/multiome-atac-gex-cisreg/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Single-Cell Multiome ATAC+GEX Peak-to-Gene Cis-Regulatory Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 206: Autonomous Single-Nucleus Multiome Simultaneous ATAC-seq and RNA-seq, Chromatin Accessibility Peak-to-Gene Linkage & TF Regulon Deconvolution
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">peak_gene_correlation_pearson</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.peak_gene_correlation_pearson}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">transcription_factor_regulon_activity_auc</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.transcription_factor_regulon_activity_auc}</p>
                </div>
              </div>
