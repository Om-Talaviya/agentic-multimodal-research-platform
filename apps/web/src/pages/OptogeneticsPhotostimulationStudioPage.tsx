...
export const OptogeneticsPhotostimulationStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Optogenetic Photostimulation Pattern Synthesis & Neuronal Spike Raster Forecaster Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/optogenetics-photostimulation/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Optogenetic Photostimulation Pattern Synthesis & Neuronal Spike Raster Forecaster Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 210: Autonomous Synthesizes spatial-temporal holographic photostimulation light patterns and forecasts channelrhodopsin kinetics, action potential firing rasters, and synaptic network entrainment.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">spike_fidelity_pct</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.spike_fidelity_pct}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">photocurrent_density_pA_um2</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.photocurrent_density_pA_um2}</p>
                </div>
              </div>
