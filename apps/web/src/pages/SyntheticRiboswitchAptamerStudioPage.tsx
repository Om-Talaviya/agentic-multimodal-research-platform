...
export const SyntheticRiboswitchAptamerStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Synthetic Bio Riboswitch Aptamer Secondary Structure & Ligand-Induced Translation Terminator Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/synthetic-riboswitch-aptamer/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Synthetic Bio Riboswitch Aptamer Secondary Structure & Ligand-Induced Translation Terminator Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 217: Autonomous Engineers synthetic theophylline and small-molecule-sensing RNA riboswitches with precise ligand-induced conformational switching between antiterminator and terminator hairpin states.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">dynamic_range_fold_induction</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.dynamic_range_fold_induction}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">switching_free_energy_kcal_mol</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.switching_free_energy_kcal_mol}</p>
                </div>
              </div>
