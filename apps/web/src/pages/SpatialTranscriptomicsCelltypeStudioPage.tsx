...
export const SpatialTranscriptomicsCelltypeStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Subcellular Spatial Transcriptomics Cell-Type Deconvolution & Niche Cell-Cell Communication Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/spatial-transcriptomics-celltype/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Subcellular Spatial Transcriptomics Cell-Type Deconvolution & Niche Cell-Cell Communication Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 215: Autonomous Performs subcellular deconvolution of multiplexed spatial transcriptomics spots into single-cell fractions and quantifies spatial ligand-receptor interaction axes in tissue niches.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">celltype_deconvolution_accuracy_pct</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.celltype_deconvolution_accuracy_pct}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">niche_colocalization_index</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.niche_colocalization_index}</p>
                </div>
              </div>
