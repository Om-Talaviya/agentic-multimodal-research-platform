...
export const SpatialTMEImmuneInfiltrationStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Pan-Cancer Spatial Tumor Microenvironment Immune Infiltration Ranker Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/spatial-tme-immune-infiltration/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Pan-Cancer Spatial Tumor Microenvironment Immune Infiltration Ranker Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 203: Autonomous Spatial Transcriptomics and Multiplexed Immunofluorescence Tumor Microenvironment (TME) Immune Infiltration, TLS Formation & Checkpoint Response Ranking
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">cd8_cytotoxic_infiltration_density</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.cd8_cytotoxic_infiltration_density}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">checkpoint_response_predictive_score</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.checkpoint_response_predictive_score}</p>
                </div>
              </div>
