...
export const WholeExomeTmbMsiRankerStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Whole-Exome Sequencing (WES) Tumor Mutation Burden (TMB) & Microsatellite Instability (MSI) Ranker Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/whole-exome-tmb-msi-ranker/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Whole-Exome Sequencing (WES) Tumor Mutation Burden (TMB) & Microsatellite Instability (MSI) Ranker Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 218: Autonomous Analyzes whole-exome somatic variant calls to compute non-synonymous Tumor Mutation Burden (TMB) per megabase and assesses microsatellite instability (MSI-High) status.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">tmb_mutations_per_mb</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.tmb_mutations_per_mb}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">msi_instability_score_pct</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.msi_instability_score_pct}</p>
                </div>
              </div>
