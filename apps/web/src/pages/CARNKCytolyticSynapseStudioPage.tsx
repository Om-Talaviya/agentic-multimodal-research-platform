...
export const CARNKCytolyticSynapseStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('CAR-NK Immune Synapse & Cytolytic Kinetics Simulator Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/car-nk-cytolytic-synapse/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              CAR-NK Immune Synapse & Cytolytic Kinetics Simulator Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 198: Autonomous Engineered Chimeric Antigen Receptor Natural Killer (CAR-NK) Immune Synapse Dynamics, Granzyme/Perforin Polarized Secretion & Multi-Target Cytolysis
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">cytolytic_specific_lysis_percent</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.cytolytic_specific_lysis_percent}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">synapse_polarization_time_minutes</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.synapse_polarization_time_minutes}</p>
                </div>
              </div>
