...
export const ImmunopeptidomeDeconvolutionStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Mass Spectrometry Immunopeptidomics & Non-Canonical Cryptic Peptide Deconvolution Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/immunopeptidome-deconvolution/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Mass Spectrometry Immunopeptidomics & Non-Canonical Cryptic Peptide Deconvolution Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 213: Autonomous Deconvolutes high-resolution Orbitrap LC-MS/MS immunopeptidome spectra, identifying non-canonical cryptic peptide antigens, trans-spliced epitopes, and HLA presentation ranks.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">spectral_identification_fdr_pct</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.spectral_identification_fdr_pct}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">presentation_affinity_nM</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.presentation_affinity_nM}</p>
                </div>
              </div>
