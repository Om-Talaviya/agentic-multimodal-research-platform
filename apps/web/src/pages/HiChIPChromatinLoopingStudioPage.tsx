...
export const HiChIPChromatinLoopingStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Epigenomic Hi-ChIP & Enhancer-Promoter Chromatin Looping Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/hichip-chromatin-looping/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Epigenomic Hi-ChIP & Enhancer-Promoter Chromatin Looping Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 200: Autonomous Epigenomic Hi-ChIP Contact Matrix Deconvolution, H3K27ac/CTCF-Mediated Enhancer-Promoter Chromatin Looping & Transcriptional Hub Discovery
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">loop_contact_enrichment_score</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.loop_contact_enrichment_score}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">enhancer_promoter_interaction_strength</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.enhancer_promoter_interaction_strength}</p>
                </div>
              </div>
