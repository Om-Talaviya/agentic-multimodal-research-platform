...
export const ASOGapmerTherapeuticsStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Antisense Oligonucleotide RNase-H Cleavage & Gapmer Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/aso-gapmer-therapeutics/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Antisense Oligonucleotide RNase-H Cleavage & Gapmer Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 193: Autonomous Antisense Oligonucleotide (ASO) 2-MOE/LNA Gapmer Architecture, RNase H1 Catalytic Cleavage & Target Knockdown Optimization
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">rnase_h_cleavage_velocity_min</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.rnase_h_cleavage_velocity_min}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">target_knockdown_percent</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.target_knockdown_percent}</p>
                </div>
              </div>
