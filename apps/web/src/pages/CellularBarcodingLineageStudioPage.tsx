...
export const CellularBarcodingLineageStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Cellular Barcoding & Lineage Tree Reconstructor Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/cellular-barcoding-lineage/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Cellular Barcoding & Lineage Tree Reconstructor Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 191: Autonomous Dynamic CRISPR Barcode Scarring, Single-Cell Lineage Tracing & Phylogenetic Maximum Parsimony Tree Reconstruction
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">lineage_tree_depth_generations</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.lineage_tree_depth_generations}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">parsimony_reconstruction_accuracy</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.parsimony_reconstruction_accuracy}</p>
                </div>
              </div>
