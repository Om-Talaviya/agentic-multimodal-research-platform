...
export const CryoEMFlexibleBackboneRefineStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Cryo-EM Continuous Flexible Backbone Motion & Deep Non-Rigid Fitting Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/cryoem-flexible-backbone-refine/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Cryo-EM Continuous Flexible Backbone Motion & Deep Non-Rigid Fitting Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 214: Autonomous Performs continuous flexible backbone deformation modeling across heterogeneous cryo-EM density maps using deep non-rigid normal mode analysis.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">density_cross_correlation</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.density_cross_correlation}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">backbone_rmsd_angstrom</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.backbone_rmsd_angstrom}</p>
                </div>
              </div>
