...
export const SpatialLipidomicsProfilingStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Spatial Lipidomics & Membrane Biogenesis Deconvolution Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/spatial-lipidomics-profiling/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Spatial Lipidomics & Membrane Biogenesis Deconvolution Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 199: Autonomous Spatial Lipidomics DESI/MALDI Mass Spectrometry Imaging, Membrane Phospholipid Saturation & Ferroptosis Lipid Peroxidation Deconvolution
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">membrane_fluidity_saturation_ratio</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.membrane_fluidity_saturation_ratio}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">ferroptosis_lipid_peroxidation_score</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.ferroptosis_lipid_peroxidation_score}</p>
                </div>
              </div>
