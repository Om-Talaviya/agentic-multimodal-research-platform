...
export const CtDNALiquidBiopsyMRDStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Pan-Cancer ctDNA Liquid Biopsy & Minimal Residual Disease Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/ctdna-liquid-biopsy-mrd/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Pan-Cancer ctDNA Liquid Biopsy & Minimal Residual Disease Engine Studio
            </h1>
          </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">mean_tumor_fraction_clearance_percent</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.mean_tumor_fraction_clearance_percent}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">mrd_recurrence_hazard_ratio</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.mrd_recurrence_hazard_ratio}</p>
                </div>
              </div>
