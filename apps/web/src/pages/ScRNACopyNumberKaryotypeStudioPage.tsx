...
export const ScRNACopyNumberKaryotypeStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Single-Cell Copy Number Variation (scCNV) & Chromosomal Aneuploidy Karyotyper Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/scrna-copy-number-karyotype/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Single-Cell Copy Number Variation (scCNV) & Chromosomal Aneuploidy Karyotyper Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 211: Autonomous Infers copy number variations and whole-chromosome aneuploidies from single-cell transcriptomic moving-average expression profiles with sub-clonal lineage tree reconstructions.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">aneuploidy_confidence_score</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.aneuploidy_confidence_score}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">breakpoint_resolution_kb</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.breakpoint_resolution_kb}</p>
                </div>
              </div>
