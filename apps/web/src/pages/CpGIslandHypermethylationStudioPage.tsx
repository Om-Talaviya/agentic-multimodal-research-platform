...
export const CpGIslandHypermethylationStudioPage: React.FC = () => {
  const [studyName, setStudyName] = useState('Autonomous Epigenomic Promoter CpG Island Hypermethylation & Tumor Suppressor Gene Silencing Engine Protocol 01');
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    try {
      const res = await fetch('/api/v1/cpg-island-hypermethylation/analyze', {
        method: 'POST',
        ...
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-400 via-sky-300 to-blue-400 bg-clip-text text-transparent">
              Autonomous Epigenomic Promoter CpG Island Hypermethylation & Tumor Suppressor Gene Silencing Engine Studio
            </h1>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Phase 212: Autonomous Quantifies promoter CpG island dense hypermethylation patterns, DNMT1/3B recruitment kinetics, and transcriptional repression of critical tumor suppressor genes.
          </p>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">silencing_repression_pct</p>
                  <p className="text-2xl font-bold text-cyan-400 mt-1">{result.silencing_repression_pct}</p>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg">
                  <p className="text-xs text-slate-500">methylation_density_beta</p>
                  <p className="text-2xl font-bold text-sky-300 mt-1">{result.methylation_density_beta}</p>
                </div>
              </div>
